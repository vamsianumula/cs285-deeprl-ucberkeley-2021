...
class AWACAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        print(ob_no.shape, ac_na.shape, re_n.shape)
        v_s = self.exploration_critic.qa_values(ob_no)
        v_ns= self.exploration_critic.qa_values(next_ob_no)
        
        re_n = ptu.from_numpy(re_n)
        
        v_ns[terminal_n==1]=0
        q = re_n + self.agent_params['learning_rate']*v_ns
        adv_n = q - v_s
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            env_reward = re_n*self.exploit_rew_scale + self.exploit_rew_shift
